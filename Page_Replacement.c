...
int lruref8bit(unsigned int input[], int nf){
	...
	int start_t = (uint)tv.tv_usec;
	
	int page_rep_clk=0, tmp_c=0;
	int clk_hnd=0;int i=0, j=0;
...
int lruRefBit(unsigned int input[], int nf) {
	...
	for (i=0; i<count_page; i++) {
		...
		unsigned int end_t=(uint)tv.tv_usec;
		time1=end_t-start_t;
	return page_rep_ref;
}
...
int optim(unsigned int input[], int nf) {
	struct timeval tv1; 
			gettimeofday(&tv1, NULL); 
			unsigned int start_t = (uint)tv1.tv_usec;
	...
	struct timeval tv3; 
		...
		unsigned int end_t=(uint)tv3.tv_usec;
			time2=end_t-start_t;
	return pg_rep_opt;
...
int lruStack(unsigned int input[], int nf) {
	struct timeval tv; 
			gettimeofday(&tv, NULL); 
			unsigned int start_t = (uint)tv.tv_usec;
	...
	struct timeval tv4; 
		...
		unsigned int end_t=(uint)tv4.tv_usec;
			time1=end_t-start_t;
...
int fifo(unsigned int input[], int nf) {
	struct timeval tv5; 
			...
			unsigned int start_t = (uint)tv5.tv_usec;
	...
	struct timeval tv6; 
		...
		unsigned int end_t=(uint)tv6.tv_usec;
